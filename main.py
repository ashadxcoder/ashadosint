...
HF_INDEX_BASE = os.environ.get(
    "ICMR_HF_INDEX_BASE",
    "https://huggingface.co/datasets/rehuuuu/icrm-hitek-full-db-mixed/resolve/main",
).rstrip("/")
...
async def search(
    ...
    # Build result without credit first
    result = {
    ...
    # Add credit at the very end
    result["credit"] = CREDIT_INFO
...
async def search_parallel(req: BatchRequest):
    ...
    # Build response without credit first
    response_data = {
        "searches": len(req.queries), 
        "results": list(results)
    }
    # Add credit at the very end
    response_data["credit"] = CREDIT_INFO
    
    return Response(content=json.dumps(response_data, indent=2, ensure_ascii=False),
